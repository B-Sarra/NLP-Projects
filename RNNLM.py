...
class RNNLM_Model(LanguageModel):
  ...
  def add_embedding(self):
    ...
    embedded = tf.nn.embedding_lookup(params=embedding, ids=self.input_placeholder)
    inputs = tf.transpose(embedded, perm=[1, 0, 2])
    inputs = tf.split(inputs, self.config.num_steps, axis=0) # splits inputs into "num_steps" sub-tensors
    for i in range(self.config.num_steps):
        inputs[i] = tf.squeeze(inputs[i], [0])
  ...
  def add_model(self, inputs):
    ...
    with tf.variable_scope('RNN', reuse=tf.AUTO_REUSE) as scope: # weight sharing
      RNN_H = tf.get_variable(shape=[self.config.hidden_size, self.config.hidden_size], dtype=tf.float32, name="RNN_H")
      RNN_I = tf.get_variable(shape=[self.config.embed_size, self.config.hidden_size], dtype=tf.float32, name="RNN_I")
      RNN_b = tf.get_variable(shape=[self.config.hidden_size, ], dtype=tf.float32, name="RNN_b")
      self.initial_state = tf.zeros(shape=[self.config.batch_size, self.config.hidden_size], dtype=tf.float32)
      self.final_state = self.initial_state
      out = []
      for input in inputs:
          self.final_state = tf.sigmoid(tf.matmul(self.final_state, RNN_H) + tf.matmul(input, RNN_I) + RNN_b)
          out.append(self.final_state)

    with tf.variable_scope('RNNDropout'):
        rnn_outputs = [tf.nn.dropout(x, self.dropout_placeholder) for x in out]
  ...
  def add_projection(self, rnn_outputs):
    ...
    with tf.variable_scope('Projection', reuse=tf.AUTO_REUSE): # weight sharing
        W = tf.get_variable(shape=[self.config.hidden_size, len(self.vocab)], dtype=tf.float32, name="W")
        b = tf.get_variable(shape=[len(self.vocab), ], dtype=tf.float32, name="b")
        outputs = []
        for rnn_output in rnn_outputs:
            outputs.append(tf.matmul(rnn_output, W) + b) #Note: softmax is already applied in starter code below
...
----------------------------------------------------------------------------------------------------
    ...
    # compute logits
    logits = output # size: [None, len(self.vocab)] = [640, 10000]
    logits = tf.reshape(logits, [self.config.batch_size, self.config.num_steps, len(self.vocab)])

    # compute targets
    targets = self.labels_placeholder
    targets = tf.reshape(targets, [self.config.batch_size, self.config.num_steps])
    targets = tf.cast(targets, dtype=tf.int32)

    # compute weights
    weights = tf.ones([self.config.batch_size, self.config.num_steps], dtype=tf.float32)
  ...
  def __init__(self, config):
    ...
    output = tf.reshape(tf.concat(self.outputs, axis=1), [-1, len(self.vocab)]) # [10, 64, 10000] ==> [640, 10000]
